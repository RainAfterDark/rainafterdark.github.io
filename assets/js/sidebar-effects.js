...
const WIND_VELOCITY_MULTIPLIER = 1.5;
...
const DROP_MIN_VELOCITY = 0.25;
const DROP_MAX_VELOCITY = 0.5;
...
var initStage = function() {
    ...
    document.addEventListener("mousemove", function checkHover(e) {
        ...
        windStrength = e.clientX / window.innerWidth;
    });
...
var resetDrop = function (drop) {
    ...
    var buffer = DROP_X_BUFFER * math.lerp(1, WIND_BUFFER_MULTIPLIER, windStrength);
    drop.x = math.randomInteger(-buffer, stage.width);
    drop.vx = math.lerp(WIND_MIN_VELOCITY, WIND_MAX_VELOCITY, windStrength);
...
var updateDrops = function (dt) {
    ...
    for (var i = drops.length - 1; i >= 0; --i) {
        var drop = drops[i];
        var vm = math.lerp(1, WIND_VELOCITY_MULTIPLIER, windStrength);
